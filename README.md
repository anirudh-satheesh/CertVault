...
## Technical Stack
- **Frontend:** React 19, Vite, Zustand, TailwindCSS v4, Framer Motion, Lucide Icons
- **Backend:** Supabase
...
Create a `.env` file in your `frontend/` directory (or use `.env.local` / `.env.production`) to specify your backend configuration:
```env
...
- `frontend/src/`: Contains the complete modular application source:
  ...
  - `services/`: Low-level API integrations.
